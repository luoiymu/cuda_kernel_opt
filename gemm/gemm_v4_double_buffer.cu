...
__global__ void gemm_v2_register(float * __restrict__ A,
    ...
    int load_smem_a_m =  idx >>1;
    int load_smem_a_k = (idx & 1) << 2;
    ...
    int load_smem_b_n= (idx & 31) << 2;//(tid % 32) * 4, col of s_b 0,4,...,124
    // 1. 再计算全局内存中的索引
...
__global__ void gemm_v3_avoid_bank_conflict(float * __restrict__ A,
                    float * __restrict__ B,
                    float * __restrict__ C,
                    const int M,
                    const int N,
    ...
    int idx = threadIdx.y*blockDim.x + threadIdx.x;
    //将smem_a换成列存储
    __shared__ float smem_a[BK][BM],smem_b[BK][BN];// 2*128*8*4=8KB
    float r_load_a[4];
    float r_load_b[4];
    float r_comp_a[TM];
    float r_comp_b[TN];
    ...
    for (int bk = 0; bk < (K + BK - 1) / BK; ++bk) {
        ...
        //列存储，并且完全展开，掩盖内存搬运延迟
        smem_a[load_smem_a_k    ][load_smem_a_m] = r_load_a[0];
        smem_a[load_smem_a_k +1 ][load_smem_a_m] = r_load_a[1];
        smem_a[load_smem_a_k +2 ][load_smem_a_m] = r_load_a[2];
        smem_a[load_smem_a_k +3 ][load_smem_a_m] = r_load_a[3];
        //为啥要从gmem -> register，再从register->smem
        FLOAT4(smem_b[load_smem_b_k][load_smem_b_n]) = FLOAT4(r_load_b[0]);
        __syncthreads();//sync load
        #pragma unroll
        for(int k=0;k<BK;k++){
            //将其分成两个块并且结果塞到register中
            FLOAT4(r_comp_a[0]) = FLOAT4(smem_a[k][threadIdx.y * TM / 2         ]);
            FLOAT4(r_comp_a[4]) = FLOAT4(smem_a[k][threadIdx.y * TM / 2 + BM / 2]);
            FLOAT4(r_comp_b[0]) = FLOAT4(smem_b[k][threadIdx.x * TN / 2         ]);
            FLOAT4(r_comp_b[4]) = FLOAT4(smem_b[k][threadIdx.x * TN / 2 + BN / 2]);
            // 3. 每个线程负责计算BM*BN(128x128)中的TM*TN(8x8)个元素
            #pragma unroll
    ...
    for (int tm = 0; tm < TM/2; ++tm) {
        int store_gmem_c_m = blockIdx.y * BM + threadIdx.y * TM/2 + tm;
        int store_gmem_c_n = blockIdx.x * BN + threadIdx.x * TN/2;
        int store_gmem_c_addr = store_gmem_c_m * N + store_gmem_c_n;
        FLOAT4(C[store_gmem_c_addr]) = FLOAT4(r_c[tm][0]);
        FLOAT4(C[store_gmem_c_addr + BN / 2]) = FLOAT4(r_c[tm][4]);
    }
    #pragma unroll
    for (int i = 0; i < TM / 2; i++) {
        int store_gmem_c_m = blockIdx.y * BM + BM / 2 + threadIdx.y * TM / 2 + i;
        int store_gmem_c_n = blockIdx.x * BN + threadIdx.x * TN / 2;
        int store_gmem_c_addr = store_gmem_c_m* N+store_gmem_c_n;
...
__global__ void gemm_v4_double_buffer(float * __restrict__ A,
                    float * __restrict__ B,
                    float * __restrict__ C,
                    const int M,
                    const int N,
    ...
    // [3]      K Tile: 将K分块，每块BK大小，迭代(K+BK-1/BK)次
    //                  每次计算TM*TN个元素各自的部分乘累加
    // [4]   Vectorize: 减少load和store指令，使用float4(利用LD/ST.128指令)
    constexpr int BM=128;
    constexpr int BN=128;
    constexpr int BK=8;
    constexpr int TM=8;
    ...
    int idx = threadIdx.y*blockDim.x + threadIdx.x;
    //将smem_a换成列存储
    __shared__ float smem_a[2][BK][BM],smem_b[2][BK][BN];// 2*128*8*4=8KB
    float r_load_a[4];
    float r_load_b[4];
    float r_comp_a[TM];
    float r_comp_b[TN];
    ...
    //
    {
        ...
        int load_gmem_a_addr = load_gmem_a_m * K + load_gmem_a_k;
        int load_gmem_b_k = load_gmem_b_k;
        int load_gmem_b_addr = load_gmem_b_k * K + load_gmem_b_n;
        FLOAT4(r_load_a[0])=FLOAT4(A[load_gmem_a_addr]);
        FLOAT4(r_load_b[0])=FLOAT4(B[load_gmem_b_addr]);
        //列存储，并且完全展开，掩盖内存搬运延迟
        smem_a[0][load_smem_a_k    ][load_smem_a_m] = r_load_a[0];
        smem_a[0][load_smem_a_k +1 ][load_smem_a_m] = r_load_a[1];
        smem_a[0][load_smem_a_k +2 ][load_smem_a_m] = r_load_a[2];
        smem_a[0][load_smem_a_k +3 ][load_smem_a_m] = r_load_a[3];
        //为啥要从gmem -> register，再从register->smem
        FLOAT4(smem_b[0][load_smem_b_k][load_smem_b_n]) = FLOAT4(r_load_b[0]);
    ...
    for (int bk = 1; bk < (K + BK - 1) / BK; ++bk) {
        ...
        for(int k=0;k<BK;k++){
            //将其分成两个块并且结果塞到register中
            FLOAT4(r_comp_a[0]) = FLOAT4(smem_a[smem_sel][k][threadIdx.y * TM / 2         ]);
            FLOAT4(r_comp_a[4]) = FLOAT4(smem_a[smem_sel][k][threadIdx.y * TM / 2 + BM / 2]);
            FLOAT4(r_comp_b[0]) = FLOAT4(smem_b[smem_sel][k][threadIdx.x * TN / 2         ]);
            FLOAT4(r_comp_b[4]) = FLOAT4(smem_b[smem_sel][k][threadIdx.x * TN / 2 + BN / 2]);
            // 3. 每个线程负责计算BM*BN(128x128)中的TM*TN(8x8)个元素
            #pragma unroll
            for(int tm=0;tm<TM;tm++) {
                #pragma unroll
        ...
        smem_a[smem_sel_next][load_smem_a_k    ][load_smem_a_m] = r_load_a[0];
        smem_a[smem_sel_next][load_smem_a_k + 1][load_smem_a_m] = r_load_a[1];
        smem_a[smem_sel_next][load_smem_a_k + 2][load_smem_a_m] = r_load_a[2];
        smem_a[smem_sel_next][load_smem_a_k + 3][load_smem_a_m] = r_load_a[3];
        FLOAT4(smem_b[smem_sel_next][load_smem_b_k][load_smem_b_n]) = FLOAT4(r_load_b[0]);
        __syncthreads();//store
    }

    #pragma unroll
    for (int tk = 0; tk < BK; tk++) {
        FLOAT4(r_comp_a[0]) = FLOAT4(smem_a[1][tk][threadIdx.y * TM / 2         ]);
        FLOAT4(r_comp_a[4]) = FLOAT4(smem_a[1][tk][threadIdx.y * TM / 2 + BM / 2]);
        FLOAT4(r_comp_b[0]) = FLOAT4(smem_b[1][tk][threadIdx.x * TN / 2         ]);
        FLOAT4(r_comp_b[4]) = FLOAT4(smem_b[1][tk][threadIdx.x * TN / 2 + BN / 2]);
...
int main(int argc ,char ** argv){
    ...
    for(int loop=0; loop<iter;loop++){
        ...
        //这里要设置对，不然后面处理逻辑就错了。
        gemm_v4_double_buffer<<<grid, block>>>(device_A, device_B, device_C, M, N,K);
    ...
    printf("gemm_v3 performance is %.2f Gflops, time=%.3f msec, Size=%.0f ops,\n",
           gigaFlops[0],
           msecPerMatrixMul[0],
           flopsPerMatrixMul);
